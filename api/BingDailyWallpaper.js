 ...
 *   <integer> [number = 1]            - number of wallpapers to retrieve (up to 8 at a time)
...
export default async (req, res) => {
    ...
    var _number = number <= 1 ? 1 : number >= 8 ? 8 : number

    // Main process
    get(`${_url}/HPImageArchive.aspx?cc=${_area}&format=js&idx=${_dateback}&n=${_number}`).then(response => {
        if (_type == "image") {
